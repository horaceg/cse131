  ...
  | Right asm_string ->
      ...
      let result =
        ...
        | Right msg -> (
            ...
            | WEXITED 0 -> Right (string_of_file rstdout_name)
            | WEXITED n ->
                Left (sprintf "Error %d: %s" n (string_of_file rstderr_name))
            | WSIGNALED n ->
...
let try_parse prog_str =
  try Right (parse_string prog_str)
  with Failure s -> Left ("Parse error: " ^ s)
...
let either_testable = Alcotest.testable either_pp compare_programs
...
let test_run program_str outfile expected (args : string list) _ =
  ...
  Alcotest.check either_testable program_str (Right (expected ^ "\n")) result

let test_err program_str outfile errmsg (args : string list) _ =
  let full_outfile = "output/" ^ outfile in
  let program = try_parse program_str in
  match program with
  | Left _ as e -> Alcotest.check either_testable program_str (Left errmsg) e
  | Right program ->
      let result = run program full_outfile args in
